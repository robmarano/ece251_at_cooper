...
class assembly_parser(object):
    ...
    # program's machine code for program, line by line
    program_code = []
    ...
    def parse_instruction(self, instruction, raw_args):
        ...
        #pp(self.instruction_table)
        #pp(machine_code)

        #print('i-{}\tb-{}'.format(instruction,machine_code))
        ...
        if len(machine_code) == 4:
            ...
            if (instruction == 'beq'):
                ...
                rt  = args[0]
            imm = offset
            print('{}\trs={}\trt={}\timm={}'.format(instruction,rs,rt,imm))
    ...
    def parse_pseudoinstruction(self, instruction, args):
        ...
        for reg_instr in instructions:
            #print('i-{}\ta-{}'.format(reg_instr, arguments[count]))
    ...
    def calculate_instruction_size(self, instruction, args):
            ...
            if instruction in self.pseudinstr_table:
                #print('i-{}\ta-{}'.format(instruction, args))
    ...
    def line_bin2hex(self, bit_string):
        bit_string = '0b'+bit_string
        hex_string = str(hex(int(bit_string, 32)))[4:]
    ...
    def store_bit_string(self, bit_string, instruction, arguments):
        ...
        for i in range(0, len(bit_string) - 1, 8):
            ...
            instr_line += self.bin2hex(bit_string[i:i + 8])
            self.current_location += 1

        # build up list of instructions in hex of program
        self.program_code.append(instr_line)
    ...
    def value_outside_range(self, value):
        ...
        #print '{} > {} = {}'.format(value,pow(2, 15) - 1,value > (pow(2, 15) - 1))
        #print '{} < {} = {}'.format(value,-(pow(2, 15)),value < -(pow(2, 15)))
    ...
    # return list of instructions as hex machine code of program
